  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-white/[0.08] pb-2.5">
        <div className="flex items-center space-x-2 text-accent-gold">
          <div className="flex h-6 w-6 items-center justify-center rounded-lg border border-accent-gold/30 bg-accent-gold/15 shadow-inner">
            {isDm ? (
              <MessageSquare className="h-3.5 w-3.5 text-accent-gold" />
            ) : (
      ...
      <div className="space-y-1.5 py-3">
        <div className="flex items-center space-x-1.5">
          ...
          <span className="font-mono text-[10px] text-slate-400">
            {isDm ? t("toast.sentDirectMessage") : t("toast.mentionedYou")}
          </span>
        </div>
        <p className="line-clamp-2 rounded-xl border border-white/[0.06] bg-surface-card/80 p-2.5 font-mono text-xs text-slate-300 italic select-text">
      ...
      <div className="flex items-center justify-end pt-1">
        ...
        >
          <span>{isDm ? t("toast.openDm") : t("toast.viewMention")}</span>
